...
export default function Products() {
  ...
  const conditionParam = (searchParams.get("condition") ?? "") as "" | "NEW" | "USED";
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      fetchProducts({
        ...
        condition: conditionParam === "NEW" || conditionParam === "USED" ? conditionParam : undefined,
        q: searchQuery || undefined,
      }),
    ])
      .then(([cats, { products: prods }]) => {
        setCategories(Array.isArray(cats) && cats.length > 0 ? cats : mockCategories);
        setProducts(Array.isArray(prods) && prods.length > 0 ? prods : mockProducts);
      })
      .catch(() => {
  ...
  }, [categoryParam, conditionParam, searchQuery]);

  if (categoryParam === "repair") return null;

  const filtered = products.filter((p) => {
    const matchCategory = !categoryParam || p.categoryId === categoryParam;
    const matchCondition = !conditionParam || p.condition === conditionParam;
    const matchSearch =
      !searchQuery ||
      ...
      <section className="py-6 sm:py-8 md:py-12">
        <div className="container-apple flex flex-col lg:flex-row gap-4 lg:gap-8">
          ...
          <aside className="hidden lg:block w-56 flex-shrink-0">
            <div className="card-premium p-6 rounded-2xl sticky top-24 border-border/50">
              ...
              <div className="space-y-4">
                <div>
                  <p className="text-sm font-medium mb-2">{t("products.condition")}</p>
                  <div className="space-y-1">
                    {(["", "NEW", "USED"] as const).map((cond) => {
                      const to = cond ? `/products?${categoryParam ? `category=${categoryParam}&` : ""}condition=${cond}` : `/products${categoryParam ? `?category=${categoryParam}` : ""}`;
                      const active = (cond === "" && !conditionParam) || (cond === conditionParam);
                      const label = cond === "" ? t("products.all") : cond === "NEW" ? t("products.conditionNew") : t("products.conditionUsed");
                      return (
                        <Link
                          key={cond || "all"}
                          to={to}
                ...
                <div>
                  ...
                  <div className="space-y-1">
                    <Link
                      to={conditionParam ? `/products?condition=${conditionParam}` : "/products"}
                      className={cn(
                        "block text-sm py-2 px-3 rounded-lg motion-smooth",
                        !categoryParam
                    ...
                    {categories.map((cat) => {
                      ...
                      const catTo = `/products?category=${cat.id}${conditionParam ? `&condition=${conditionParam}` : ""}`;
                      return (
                        <Link
                          key={cat.id}
                          to={catTo}
          ...
          <div className="flex-1 min-w-0">
            {showFilters && (
              <div className="lg:hidden card-premium p-5 rounded-2xl mb-6 space-y-4">
                <div>
                  <p className="text-sm font-medium mb-2">{t("products.condition")}</p>
                  <div className="flex flex-wrap gap-2">
                    {(["", "NEW", "USED"] as const).map((cond) => {
                      ...
                      const label = cond === "" ? t("products.all") : cond === "NEW" ? t("products.conditionNew") : t("products.conditionUsed");
                      return (
                        <Link key={cond || "all"} to={to} className={cn("px-3 py-1.5 rounded-lg text-sm motion-smooth", active ? "bg-primary text-primary-foreground" : "bg-secondary")}>
                          {label}
                        </Link>
                      );
                    })}
                ...
                <div>
                  <p className="text-sm font-medium mb-2">{t("products.category")}</p>
                  <div className="flex flex-wrap gap-2">
                    <Link to={conditionParam ? `/products?condition=${conditionParam}` : "/products"} className={cn("px-3 py-1.5 rounded-lg text-sm motion-smooth", !categoryParam ? "bg-primary text-primary-foreground" : "bg-secondary")}>
                      {t("products.all")}
                    </Link>
                    {categories.map((cat) => {
                      const label = cat.id === "repair" ? t("categories.repair.name") : cat.name;
                      const catTo = `/products?category=${cat.id}${conditionParam ? `&condition=${conditionParam}` : ""}`;
                      return (
                        <Link key={cat.id} to={catTo} className={cn("inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-sm motion-smooth", categoryParam === cat.id ? "bg-primary text-primary-foreground" : "bg-secondary")}>
            ...
            {loading ? (
              <div className="grid grid-cols-2 lg:grid-cols-2 xl:grid-cols-3 gap-3 sm:gap-4 md:gap-6">
                {[1, 2, 3, 4, 5, 6].map((i) => (
                  <div key={i} className="rounded-2xl border border-border/60 bg-card h-80 animate-pulse" />
                ))}
              </div>
            ) : sorted.length === 0 ? (
              <p className="text-muted-foreground text-center py-12">{t("products.noProducts")}</p>
            ) : (
              <div className="grid grid-cols-2 lg:grid-cols-2 xl:grid-cols-3 gap-3 sm:gap-4 md:gap-6">
                {sorted.map((product) => (
